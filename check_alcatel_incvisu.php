...
class Telnet
{
    ...
    public function login($username, $password, $host_type = 'linux')
    {
        ...
        try {
            ...
            throw new \Exception("Login failed.".print_r($e, true));
        }
...
$t = new Telnet($config['ip'], 23, 30);
if ($t->login($config['username'], $config['password']) == Telnet::TELNET_OK) {
    $return = $t->exec('incvisu -T 50 | grep FlexLM');
	...
	if(strpos($return[count($return)-1], "breakdown") !== false) {
		
		$dateRaw = explode(" ", $return[count($return)-1]);
		$dt = DateTime::createFromFormat("d/m/y H:i:s", $dateRaw[0]." ".$dateRaw[1]);
		$incDate = $dt->getTimestamp();
		
		#echo "Jetzt: ".time().PHP_EOL;
		#echo "Inc: ".$incDate.PHP_EOL;
		#var_dump((time() - (int)$incDate));
		if((time() - (int)$incDate) > 300 ) {
			$ret = 0;
			echo "There was an recent incident, but its older than 5 minutes (Alcatel OXE checks every 3 minutes), so we assume FlexLM is working again - ";
		} else {
			$ret = 2;
		}
	} elseif(strpos($return[count($return)-1], "successful") !== false || strpos($return[count($return)-1], "incvisu -T") ) {
		$ret = 0;
	} else {
		$ret = 3;
	}
	
	echo str_replace("|", " - ", $return[count($return)-1]);
